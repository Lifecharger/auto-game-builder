...
# Thumbnail cache — portable location
_THUMB_CACHE_DIR = Path(tempfile.gettempdir()) / "auto_game_builder" / "pipeline_thumbs"
...
async def lifespan(app: FastAPI):
    ...
    # Initialize asset pipeline engine
    try:
        from core.pipeline_engine import PipelineEngine
        app.state.pipeline = PipelineEngine(app.state.db, settings)
    except Exception as e:
...
def pipeline_list_pairs():
    """List all pairs in grok-favorites with extras info."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
...
def pipeline_create_session(body: PipelineSessionCreate):
...
def pipeline_list_assets(
    ...
    """List pipeline assets with optional filters."""
    assets = db().get_pipeline_assets(
        session_id=session_id, status=status, rating=rating,
        collection=collection, offset=offset, limit=limit,
    )
    return [_pipeline_asset_dict(a) for a in assets]


@app.get("/api/pipeline/assets/{asset_id}")
def pipeline_get_asset(asset_id: int):
    """Get a single pipeline asset with all metadata."""
    a = db().get_pipeline_asset(asset_id)
    if not a:
        raise HTTPException(404, "Asset not found")
    return _pipeline_asset_dict(a)


@app.post("/api/pipeline/match")
def pipeline_start_match(body: PipelineMatchRequest):
    """Start background video-image matching. No session_id needed."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
...
def pipeline_start_tag(body: PipelineTagRequest):
...
def pipeline_accept(body: PipelineAcceptRequest):
    """Accept pairs into a collection. Supports pairs (filename-based) or asset_ids."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
    pairs_dicts = [p.model_dump() for p in body.pairs] if body.pairs else None
    result = pe.accept_assets(
        asset_ids=body.asset_ids, collection=body.collection,
...
def pipeline_reject(body: PipelineRejectRequest):
    """Reject/delete pipeline assets by asset_ids or filenames."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
    if body.filenames:
...
def pipeline_generate_music(body: PipelineMusicRequest):
    """Generate background music for a collection via ElevenLabs."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
...
def pipeline_start_push(body: PipelinePushRequest):
    """Start background R2 push."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
...
def pipeline_list_collections(rating: Optional[str] = None):
    """List pipeline collections. If no rating, returns all ratings."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
    if rating:
        return pe.list_collections(rating)
    # Return all ratings when no filter specified
    all_collections = []
...
def pipeline_create_collection(body: PipelineCollectionCreate):
...
def pipeline_collection_files(collection: str, rating: str = "teen", pushed: bool = False):
    """List actual files in a collection folder."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
...
def pipeline_serve_collection_file(collection: str, filename: str, rating: str = "teen", pushed: bool = False):
    """Serve a file from a collection folder."""
    ...
    try:
        ...
        if not pe:
            raise HTTPException(503, "Pipeline engine not initialized")
        config = pe.RATING_CONFIGS.get(rating, pe.RATING_CONFIGS["teen"])
        folder_name = config["folder"]
        if pushed:
            gen_root = pe.PIPELINE_BASE / f"{folder_name} - Pushed" / "Generations"
        else:
            gen_root = pe.PIPELINE_BASE / folder_name / "Generations"
        ...
        return FileResponse(str(file_path), media_type=media_map.get(ext, "application/octet-stream"), headers=headers)
...
def pipeline_collection_file_thumb(collection: str, filename: str, rating: str = "teen", pushed: bool = False, size: int = 200):
    """Serve thumbnail of a collection file."""
    ...
    try:
        ...
        cache_key = f"col_{rating}_{collection}_{filename}_{size}"
        cache_file = cache_dir / f"{cache_key}.jpg"

        if cache_file.exists() and cache_file.stat().st_mtime >= file_path.stat().st_mtime:
            headers = {"Cache-Control": "public, max-age=3600"}
            return FileResponse(str(cache_file), media_type="image/jpeg", headers=headers)
...
def pipeline_op_status(op_id: str):
    """Get pipeline operation status."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
...
def pipeline_cancel_op(op_id: str):
...
def pipeline_serve_file(filename: str):
    """Serve a file from grok-favorites by filename. Full resolution — use /thumb for grid."""
    from fastapi.responses import FileResponse
    try:
        if ".." in filename:
            raise HTTPException(400, "Invalid filename")
        file_path = Path(os.path.join(str(Path.home()), "Downloads", "grok-favorites")) / filename
        if not file_path.exists():
            raise HTTPException(404, "File not found")
        ext = file_path.suffix.lower()
        media_map = {
        ...
        # Cache for 1 hour — file content changes only on match/rename
        headers = {"Cache-Control": "public, max-age=3600"}
        return FileResponse(str(file_path), media_type=media_map.get(ext, "application/octet-stream"), headers=headers)
    except HTTPException:
        raise
...
def pipeline_serve_thumbnail(filename: str, size: int = 200):
    """Serve a small thumbnail of an image from grok-favorites. Max 400px."""
    from fastapi.responses import Response, FileResponse
    try:
        if ".." in filename:
            raise HTTPException(400, "Invalid filename")
        ...
        size = min(size, 400)  # Cap at 400px
...
def pipeline_file_metadata(filename: str):
    """Read EXIF metadata from image or .json sidecar from video."""
    try:
        if ".." in filename:
            raise HTTPException(400, "Invalid filename")
        file_path = Path(os.path.join(str(Path.home()), "Downloads", "grok-favorites")) / filename
        ...
        # For videos or if no EXIF: try .json sidecar
...
def pipeline_asset_thumbnail(asset_id: int):
    """Serve 256px JPEG thumbnail for an asset."""
    from fastapi.responses import FileResponse
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
...
def pipeline_asset_image(asset_id: int):
    """Serve full-resolution image for an asset."""
    from fastapi.responses import FileResponse
    a = db().get_pipeline_asset(asset_id)
    ...
    return FileResponse(a.file_path, media_type=media_map.get(ext, "application/octet-stream"))


def _pipeline_asset_dict(a) -> dict:
    """Convert PipelineAsset to JSON-serializable dict."""
    metadata = {}
    try:
...
def pipeline_catalog_import():
    """Import all existing assets into the catalog database."""
    pe = pipeline()
    if not pe:
        raise HTTPException(503, "Pipeline engine not initialized")
    try:
...
def pipeline_catalog_list(
    ...
    d = db()
    assets = d.get_catalog_assets(rating=rating, collection=collection, limit=limit, offset=offset)
    total = d.count_catalog_assets(rating=rating, collection=collection)
    return {
        "total": total,
        "offset": offset,
        "limit": limit,
...
def pipeline_catalog_collections(rating: Optional[str] = None):
    """List all catalog collections with asset counts."""
    d = db()
    return d.get_catalog_collections(rating=rating)
