...
### General
- Run appropriate lightweight checks after changes. Request any necessary build through the AGB build pipeline; all builds and heavy local work must use its gpu_lane queue. Never run Flutter, Godot, Unity or Gradle builds directly. A timeout or recently modified artifact alone is not proof of success: report the pipeline result. Inside a pipeline repair step, finish the code fix and let the owning pipeline retry; do not wait for a nested build on the same lane.
