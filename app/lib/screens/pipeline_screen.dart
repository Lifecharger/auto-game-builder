...
class _PipelineScreenState extends State<PipelineScreen> with WidgetsBindingObserver {
  ...
  bool _initialLoadDone = false;   // True after first data fetch

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addObserver(this);
    // Don't load data eagerly — wait until the pipeline tab is active.
    // We listen for tab changes via AppState and trigger loading on first activation.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final appState = context.read<AppState>();
      appState.addListener(_onAppStateChanged);
      // If already on the pipeline tab at startup (unlikely but possible)
      _onAppStateChanged();
    });
  }
  ...
  void dispose() {
    ...
    // Remove the AppState listener to avoid leaks
    try {
  ...
  /// Listener attached to AppState — triggers first load when pipeline tab is activated.
  void _onAppStateChanged() {
    if (!mounted) return;
    final isActive = context.read<AppState>().activeTabIndex == _myTabIndex;
    if (isActive && !_initialLoadDone) {
  ...
  void _startPolling() {
    _pollTimer?.cancel();
    // Only poll active operations — don't reload pairs/collections constantly
    // User can pull-to-refresh to reload data on demand
    final hasActiveOps = _activeOps.any((op) => !op.isDone);
    if (!hasActiveOps) return; // No polling needed when idle
    _pollTimer = Timer.periodic(const Duration(seconds: 3), (_) {
      ...
      if (_activeOps.any((op) => !op.isDone)) {
        ...
        // Ops just finished — reload data once and stop polling
  ...
  Future<void> _loadPairs({bool silent = false}) async {
    final result = await ApiService.getPipelinePairs();
  ...
  Future<void> _loadCollections({bool silent = false}) async {
    final result = await ApiService.getPipelineCollections();
    if (mounted && result.ok) {
      setState(() {
  ...
  Future<void> _pollOps() async {
    ...
    for (final op in _activeOps) {
      ...
      final result = await ApiService.getPipelineOpStatus(op.opId);
      if (result.ok) {
  ...
  Future<void> _scan() async {
    ...
    final scanResult = await ApiService.scanPipeline();
  ...
  Future<void> _matchSelected() async {
    ...
    final result = await ApiService.startPipelineMatch(
  ...
  Future<void> _tagSelected() async {
    final result = await ApiService.startPipelineTag();
    if (mounted) {
      if (result.ok) {
        final opId = result.data!['op_id']?.toString() ?? '';
            ...
            if (image.isNotEmpty)
              Padding(
                ...
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: Image.network(
                    ApiService.pipelineFileThumbUrl(image),
                    height: 120,
                    fit: BoxFit.cover,
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: submitting ? null : () async {
                        ...
                        final result = await ApiService.acceptPipelinePairs(
                          pairs: acceptPairs,
                          collection: collName,
                          rating: acceptRating,
                        );
  ...
  Future<void> _rejectSelected() async {
    ...
    final result = await ApiService.rejectPipelineAssets(filenames);
    if (mounted) {
  ...
  Future<void> _pushCollection(PipelineCollectionModel coll) async {
    final result = await ApiService.pushPipelineCollection(
  ...
  Future<void> _cancelOp(PipelineOpStatus op) async {
    final result = await ApiService.cancelPipelineOp(op.opId);
    if (mounted) {
              ...
              actions: [
                ...
                FilledButton(
                  onPressed: submitting ? null : () async {
                    ...
                    final result = await ApiService.createPipelineCollection(
              ...
              onPlayVideo: () {
                ...
                Navigator.push(context, MaterialPageRoute(
                  builder: (_) => _VideoPlayerPage(
                    videoUrl: ApiService.pipelineFileUrl(videoFile!),
                    title: 'Pair $stem',
                  ),
                ));
              },
  ...
  Future<void> _loadMetadata() async {
    try {
      if (widget.imageFile != null) {
        final r = await ApiService.getPipelineFileMetadata(widget.imageFile!);
        if (r.ok && mounted) setState(() => _imageMetadata = r.data);
      }
      if (widget.videoFile != null) {
        final r = await ApiService.getPipelineFileMetadata(widget.videoFile!);
        ...
        children: [
          ...
          if (widget.imageFile != null) ...[
            ClipRRect(
              ...
              child: Image.network(
                ApiService.pipelineFileUrl(widget.imageFile!),
                fit: BoxFit.contain,
                width: double.infinity,
      ...
      child: Card(
        ...
        child: Stack(
          ...
          children: [
            ...
            if (imageFile != null)
              Image.network(
                ApiService.pipelineFileThumbUrl(imageFile!),
                fit: BoxFit.contain,
                cacheWidth: 200,
...
class _CollectionCardState extends State<_CollectionCard> {
  ...
  Future<void> _loadCollectionFiles() async {
    ...
    final result = await ApiService.getCollectionFiles(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (ctx, index) {
                    ...
                    return ClipRRect(
                      borderRadius: BorderRadius.circular(6),
                      child: Image.network(
                        ApiService.pipelineCollectionFileThumbUrl(
                          widget.collection.name,
                          filename,
                          widget.collection.rating,
                          pushed: widget.collection.isPushed,
    ...
    _controller = VideoPlayerController.networkUrl(Uri.parse(widget.videoUrl))
      ..initialize().then((_) {
        if (mounted) {
          ...
          // Don't auto-play — show first frame and let user tap to play.
        }
      }).catchError((e) {
