...
class ApiService {
  ...
  static Future<ApiResult<Map<String, dynamic>>> createPipelineSession({
    required String sourceFolder,
    String rating = 'teen',
  }) async {
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/sessions'),
              headers: _headers,
              body: jsonEncode({'source_folder': sourceFolder, 'rating': rating}))
          .timeout(const Duration(seconds: 30));
      if (response.statusCode == 200) {
        return ApiResult.success(jsonDecode(response.body));
      }
  ...
  static Future<ApiResult<List<dynamic>>> getPipelineSessions() async {
    try {
      final response = await _getWithRetry(Uri.parse('$_base/api/pipeline/sessions'));
      if (response.statusCode == 200) {
        return ApiResult.success(jsonDecode(response.body));
      }
      return ApiResult.failure(_httpError(response.statusCode));
  ...
  static Future<ApiResult<List<dynamic>>> getPipelineAssets({
    int? sessionId,
    String? status,
    String? rating,
    String? collection,
    ...
    try {
      ...
      final uri = Uri.parse('$_base/api/pipeline/assets').replace(queryParameters: params);
      final response = await _getWithRetry(uri);
      if (response.statusCode == 200) {
        return ApiResult.success(jsonDecode(response.body));
      }
  ...
  static Future<ApiResult<Map<String, dynamic>>> getPipelineAsset(int assetId) async {
    try {
      final response = await _getWithRetry(Uri.parse('$_base/api/pipeline/assets/$assetId'));
      if (response.statusCode == 200) {
        return ApiResult.success(jsonDecode(response.body));
      }
      return ApiResult.failure(_httpError(response.statusCode));
  ...
  static Future<ApiResult<Map<String, dynamic>>> startPipelineMatch({
    bool checkFirstFrame = true,
  }) async {
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/match'),
              headers: _headers,
              body: jsonEncode({
                'rename': true,
                'check_first_frame': checkFirstFrame,
              }))
          .timeout(const Duration(seconds: 30));
      if (response.statusCode == 200) {
        return ApiResult.success(jsonDecode(response.body));
  ...
  static Future<ApiResult<Map<String, dynamic>>> getPipelineFileMetadata(String filename) async {
    try {
      final response = await http
          .get(Uri.parse('$_base/api/pipeline/file/${Uri.encodeComponent(filename)}/metadata'), headers: _headers)
          .timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
        return ApiResult.success(jsonDecode(response.body) as Map<String, dynamic>);
      }
  ...
  static Future<ApiResult<Map<String, dynamic>>> reassignPipelineAsset({
    required int assetId,
    required int targetImageId,
  }) async {
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/reassign'),
              headers: _headers,
              body: jsonEncode({
                'asset_id': assetId,
  ...
  static Future<ApiResult<Map<String, dynamic>>> startPipelineTag({
    bool force = false,
  }) async {
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/tag'),
  ...
  static Future<ApiResult<List<dynamic>>> getPipelinePairs() async {
    try {
      final response = await http
          .get(Uri.parse('$_base/api/pipeline/pairs'), headers: _headers)
          .timeout(const Duration(seconds: 15));
      if (response.statusCode == 200) {
  ...
  static Future<ApiResult<Map<String, dynamic>>> acceptPipelinePairs({
    ...
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/accept'),
              headers: _headers,
              body: jsonEncode({
                'pairs': pairs,
  ...
  static Future<ApiResult<Map<String, dynamic>>> rejectPipelineAssets(List<String> filenames) async {
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/reject'),
              headers: _headers,
  ...
  static Future<ApiResult<Map<String, dynamic>>> generatePipelineMusic({
    ...
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/generate-music'),
              headers: _headers,
              body: jsonEncode({
                'collection': collection,
                'rating': rating,
  ...
  static Future<ApiResult<Map<String, dynamic>>> pushPipelineCollection({
    ...
    try {
      ...
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/push'),
  ...
  static Future<ApiResult<Map<String, dynamic>>> createPipelineCollection({
    ...
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/collections'),
              headers: _headers,
              body: jsonEncode({'name': name, 'rating': rating}))
          .timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
        return ApiResult.success(jsonDecode(response.body));
      }
  ...
  static Future<ApiResult<bool>> cancelPipelineOp(String opId) async {
    try {
      final response = await http
          .post(Uri.parse('$_base/api/pipeline/ops/$opId/cancel'), headers: _headers)
          .timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) return const ApiResult.success(true);
      return ApiResult.failure(_httpError(response.statusCode));
    } catch (e) {
  ...
  static String pipelineCollectionFileThumbUrl(String collection, String filename, String rating, {bool pushed = false, int size = 200}) =>
