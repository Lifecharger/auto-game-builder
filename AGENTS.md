...
### GDD & CLAUDE.md & AGENTS.md & Art Bible
| Method | Path | Body |
|--------|------|------|
...
| GET | `/api/apps/{app_id}/agents-md` | — → `{content, exists}` (reads project-root `AGENTS.md`) |
| PUT | `/api/apps/{app_id}/agents-md` | `{content}` → `{status:"ok"}` (atomic UTF-8 save) |
| GET | `/api/apps/{app_id}/art-bible` | — → `{content, exists}` (reads `design/art-bible.md`) |
| PUT | `/api/apps/{app_id}/art-bible` | `{content}` → `{ok:true}` |
| POST | `/api/apps/{app_id}/enhance` | `{type}` where `type` ∈ `gdd` · `claude-md` · `agents-md` · `art-bible` |
...
1. Request builds through the AGB pipeline and its GPU lane. Default to AAB on the internal channel with upload enabled (`track: internal`, `build_target: aab`, `upload: true`); use APK only when the user explicitly requests it.
2. Use an emulator or launch a GUI only when the user explicitly requests it. The user normally tests on a real phone.
3. Run the relevant automated tests; distinguish automated results from device checks.
...
### Regression suite and runtime rules (task #365)
- Backend: `python -m unittest discover -s server/tests -v` from the project root. Tests isolate functions and never import the live Comfy dispatcher or write live job state.
- Mobile: `flutter test --no-pub --concurrency=1` from `app/`. Run analysis with zero errors; record existing informational lints separately.
- Every build uses the AGB pipeline / `gpu_lane`; agents must not start direct Flutter, Godot, Unity or Gradle builds.
- Preserve dispatcher-owned lane tickets during reorder. Do not release a cancelled Comfy job's lane until its prompt ID is absent from ComfyUI's queue.
- Keep build cancellation set until the worker exits. New attempts may not overlap a cancelled attempt's cleanup.
- `/api/queue` waiting tickets expose `can_move_up` and `can_move_down`; `build:<app_id>` job IDs route cancellation to the build engine.
- `/api/sync` replay cursors precede snapshot reads; delta timestamps intentionally overlap their second.
- Generated audio is a distinct media type, not an image source. See `gdd.md` for the current queue/media contracts.
- Maintain `AGENTS.md` as an exact copy of this project's `CLAUDE.md`.
...
AGB is a public repository. The release pipeline automatically commits and pushes: inspect pending changes before building, and keep secrets and machine-specific paths out of source, logs and commits.
